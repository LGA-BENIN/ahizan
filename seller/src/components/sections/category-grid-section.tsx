...
export function CategoryGridSection({ title }: CategoryGridSectionProps) {
    return (
        <div className="bg-muted/30 py-12 md:py-16 lg:py-24">
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 gap-y-6 sm:grid-cols-2 lg:grid-cols-3 gap-x-6">
                    ...
                    {['Mode', 'Électronique', 'Maison', 'Beauté', 'Sport', 'Artisanat Local'].map((cat, i) => (
                        <Link key={i} href={`/collections/${cat.toLowerCase()}`} className="group relative flex h-64 w-full flex-col overflow-hidden rounded-2xl bg-card border hover:shadow-xl transition-all">
