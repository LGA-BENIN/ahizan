const { bootstrapWorker, RequestContext, ChannelService, OrderService, Order } = require('@vendure/core');
const { config } = require('./dist/vendure-config');

async function run() {
    console.log('Bootstrapping Vendure worker context...');
    ...
    console.log('Found Order 93:', order.code, 'Current State:', order.state);
    ...
    setTimeout(async () => {
        const logs = await connection.rawConnection.query(`
            ...
            ORDER BY id ASC;
        `);
        console.log('\n--- NEW NOTIFICATION LOG ENTRIES CREATED ---');
        console.table(logs);
        process.exit(0);
    }, 3000);
}
