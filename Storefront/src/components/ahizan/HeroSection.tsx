    ...
    return (
        <div className="flex flex-col lg:flex-row gap-3 md:gap-4">
            ...
            {heroConfig.showSidebar && (
                <aside className={`hidden lg:flex w-60 border border-border/60 rounded-2xl self-stretch bg-white shadow-sm`}>
                                ...
                                <div key={cat.id || i} className="group/cat relative">
                                    ...
                                    >
                                        ...
                                        {cat.children?.length > 0 && (
                                            <ChevronDown className="w-3 h-3 text-muted-foreground transition-transform group-hover/cat:rotate-180" />
                                        )}
                                    </Link>
                                    {/* Sub-collection flyout on hover */}
                                    {cat.children?.length > 0 && (
                                        <div className="absolute left-full top-0 hidden group-hover/cat:block bg-white border border-border/60 rounded-xl shadow-lg py-2 min-w-[200px] z-50">
                                            {cat.children.map((sub: any) => (
                                                <Link
                                                    key={sub.id}
                                                    href={`/collection/${sub.slug}`}
                                                    className="flex items-center gap-2 px-4 py-2 text-[12px] text-foreground/70 hover:text-primary hover:bg-muted/30 transition-all whitespace-nowrap"
                                                >
                                                    {sub.icon ? (
                                                        <img src={sub.icon} className="w-3.5 h-3.5 object-cover rounded-sm" alt="" />
                                                    ) : (
